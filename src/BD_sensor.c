...
void adust_Z_live(uint16_t sensor_z)
{
   // BD_Data
	...
	output("Z_Move_L mcuoid=%c diff_stp=%c", oid_g,(cur_stp-step_adj[0].steps_at_zero));
    if((step_adj[0].adj_z_range*step_adj[0].steps_per_mm/10)<=(cur_stp-step_adj[0].steps_at_zero))
		return;
	if(sensor_z>=390)
		return;//out of range
    int diff_step=sensor_z*step_adj[0].steps_per_mm/100-(cur_stp-step_adj[0].steps_at_zero);
	int dir=0;
...
//	memset(data,0,16);
	...
	if(dir==1)	
	{
		if((cur_stp>=interD)&&((cur_stp_old<interD)||(stepx_probe.x_count==0)))
		{
			...
			output("report_x_probe mcuoid=%c cur_stp_old0=%c", oid_g,cur_stp_old);			
...
command_Z_Move_Live(uint32_t *args)
{
	...
	else if(tmp[0]=='6')
	{
	    ...
	    uint32_t cur_stp=stepper_get_position(s);
		step_adj[0].zoid=j;
		step_adj[0].steps_at_zero=cur_stp-(step_adj[0].steps_at_zero*step_adj[0].steps_per_mm)/1000;
